...
func TestRequireAdminAPI(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			user:       &store.User{ID: "u1", Role: store.RoleMember},
		...
		},
		{
			...
			user:       &store.User{ID: "u2", Role: store.RoleAdmin},
			wantStatus: http.StatusOK,
			wantPass:   true,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			RequireAdminAPI(inner).ServeHTTP(w, req)

			if w.Code != tc.wantStatus {
				t.Errorf("status = %d, want %d", w.Code, tc.wantStatus)
			}
			if passedThrough != tc.wantPass {
				t.Errorf("passed through = %v, want %v", passedThrough, tc.wantPass)
			}
