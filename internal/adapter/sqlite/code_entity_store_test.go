...
func setupCodeEntityStore(t *testing.T) store.CodeEntityStore {
	t.Helper()
	db := setupTestDB(t)
...
func TestCodeEntityUpsert(t *testing.T) {
	...
	e, err := s.Upsert(ctx, store.UpsertCodeEntityParams{
	...
	e2, err := s.Upsert(ctx, store.UpsertCodeEntityParams{
		EntityType: store.CodeEntityFile,
		EntityName: "app/controllers/users_controller.rb",
		Service:    "api",
	})
...
func TestCodeEntityGetByName(t *testing.T) {
	...
	_, err := s.Upsert(ctx, store.UpsertCodeEntityParams{
		EntityType: store.CodeEntityController,
	...
	e, err := s.GetByName(ctx, store.CodeEntityController, "UsersController", "web")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	...
	// Not found
	_, err = s.GetByName(ctx, store.CodeEntityController, "Missing", "web")
...
func TestCodeEntityIncrementError(t *testing.T) {
	...
	if err := s.IncrementError(ctx, store.CodeEntityFile, "users.rb", "api"); err != nil {
	...
	e, err := s.GetByName(ctx, store.CodeEntityFile, "users.rb", "api")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	if e.ErrorCount != 2 {
...
func TestCodeEntityIncrementInvestigation(t *testing.T) {
	...
	if err := s.IncrementInvestigation(ctx, store.CodeEntityFile, "orders.rb", "api"); err != nil {
		t.Fatalf("increment: %v", err)
	}

	e, err := s.GetByName(ctx, store.CodeEntityFile, "orders.rb", "api")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
...
func TestCodeEntityTopByRisk(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		for j := 0; j <= i; j++ {
			if err := s.IncrementError(ctx, store.CodeEntityFile, fmt.Sprintf("file%d.rb", i), "api"); err != nil {
				t.Fatalf("increment: %v", err)
			}
		}
	}
...
func TestCodeEntityBatchGetRisk(t *testing.T) {
	...
	if err := s.IncrementError(ctx, store.CodeEntityFile, "a.rb", "api"); err != nil {
		t.Fatalf("increment: %v", err)
	}
	if err := s.IncrementError(ctx, store.CodeEntityFile, "b.rb", "api"); err != nil {
		t.Fatalf("increment: %v", err)
	}

	entities, err := s.BatchGetRisk(ctx, store.CodeEntityFile, []string{"a.rb", "b.rb", "missing.rb"}, "api")
	if err != nil {
...
func TestCodeEntityPrune(t *testing.T) {
	...
	if err := s.IncrementError(ctx, store.CodeEntityFile, "active.rb", "api"); err != nil {
		t.Fatalf("increment: %v", err)
	}
