...
func TestScheduler_StartStop(t *testing.T) {
	...
	sched := NewScheduler(SchedulerOpts{
		...
		AlertStore:    as,
		Registry:      registry,
		LLMProvider:   &mockLLMProvider{response: llm.ChatResponse{Content: "OK: all good"}},
...
func TestScheduler_ExecutesDueWatchers(t *testing.T) {
	...
	sched := NewScheduler(SchedulerOpts{
		...
		AlertStore:    as,
		Registry:      registry,
		LLMProvider:   &mockLLMProvider{response: llm.ChatResponse{Content: "ALERT: found issues"}},
		AgentCfg:      agent.RunConfig{MaxSteps: 3, MaxToolCalls: 2, MaxObservationBytes: 4096},
		PollInterval:  50 * time.Millisecond,
		MaxConcurrent: 1,
	})
...
func TestScheduler_ContextCancellation(t *testing.T) {
	...
	sched := NewScheduler(SchedulerOpts{
		...
		AlertStore:    as,
		Registry:      registry,
		LLMProvider:   &mockLLMProvider{response: llm.ChatResponse{Content: "OK: fine"}},
		AgentCfg:      agent.RunConfig{MaxSteps: 3, MaxToolCalls: 2, MaxObservationBytes: 4096},
