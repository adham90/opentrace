...
func TestSettingsStore_SetAndGet(t *testing.T) {
	...
	err := ss.SetRetention(ctx, store.RetentionSettings{RetentionDays: 7})
	if err != nil {
...
func TestSettingsStore_KeepForever(t *testing.T) {
	...
	err := ss.SetRetention(ctx, store.RetentionSettings{RetentionDays: 0})
	if err != nil {
		t.Fatalf("SetRetention: %v", err)
	}
...
func TestSettingsStore_Upsert(t *testing.T) {
	...
	// First write
	err := ss.SetRetention(ctx, store.RetentionSettings{RetentionDays: 14})
	if err != nil {
		t.Fatalf("SetRetention first: %v", err)
	}

	// Second write overwrites
	err = ss.SetRetention(ctx, store.RetentionSettings{RetentionDays: 60})
	if err != nil {
