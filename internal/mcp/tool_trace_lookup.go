...
// traceLookupHandler returns a handler that follows a distributed trace across
// services, assembling a timeline with timing and error information.
func traceLookupHandler(ls store.LogStore) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		entries, err := ls.Search(ctx, store.LogSearchParams{
			TraceID: traceID,
			Limit:   1000,
		})
		if err != nil {
		...
		if includeContext && len(entries) > 0 {
			...
			for svc, stats := range svcMap {
				...
				ctxLogs, err := ls.Search(ctx, store.LogSearchParams{
					Service: svc,
					Start:   &ctxStart,
					End:     &ctxEnd,
					Limit:   50,
