...
// ---------------------------------------------------------------------------
// Tests: LinkInvestigationToEntities
// ---------------------------------------------------------------------------

func TestLinkInvestigationToEntities_NilStores(t *testing.T) {
	ctx := context.Background()
	sess := &store.InvestigationSession{
		InvestigatedErrorFingerprints: []string{"fp1"},
	}

	// None of these should panic.
	LinkInvestigationToEntities(ctx, nil, nil, sess)
	LinkInvestigationToEntities(ctx, &mockCodeEntityStore{}, nil, sess)
	LinkInvestigationToEntities(ctx, nil, &mockErrorGroupStore{}, sess)
}

func TestLinkInvestigationToEntities_NilSession(t *testing.T) {
	ctx := context.Background()
	// Must not panic.
	LinkInvestigationToEntities(ctx, &mockCodeEntityStore{}, &mockErrorGroupStore{}, nil)
}

func TestLinkInvestigationToEntities_WithFingerprints(t *testing.T) {
	ctx := context.Background()

	ces := &mockCodeEntityStore{}
	egs := &mockErrorGroupStore{
		groups: map[string]*store.ErrorGroup{
			"fp-auth": {
				Fingerprint: "fp-auth",
				Service:     "auth-svc",
				SourceFile:  "app/auth/login.rb",
			},
			"fp-pay": {
				Fingerprint: "fp-pay",
				Service:     "payment-svc",
				SourceFile:  "app/payment/charge.go",
			},
			"fp-no-source": {
				Fingerprint: "fp-no-source",
				Service:     "web",
				SourceFile:  "", // no source file — should be skipped
			},
		},
	}

	sess := &store.InvestigationSession{
		InvestigatedErrorFingerprints: []string{"fp-auth", "fp-pay", "fp-no-source", "fp-unknown"},
	}

	LinkInvestigationToEntities(ctx, ces, egs, sess)

	// Expect 2 calls: fp-auth and fp-pay. fp-no-source is skipped (empty SourceFile),
	// fp-unknown is skipped (Get returns error).
	if len(ces.incrementInvestCalls) != 2 {
		t.Fatalf("expected 2 IncrementInvestigation calls, got %d: %+v",
			len(ces.incrementInvestCalls), ces.incrementInvestCalls)
	}

	callMap := make(map[string]struct {
		typ     store.CodeEntityType
		service string
	})
	for _, c := range ces.incrementInvestCalls {
		callMap[c.name] = struct {
			typ     store.CodeEntityType
			service string
		}{c.typ, c.service}
	}

	if v, ok := callMap["app/auth/login.rb"]; !ok {
		t.Error("missing IncrementInvestigation call for app/auth/login.rb")
	} else {
		if v.typ != store.CodeEntityFile {
			t.Errorf("type = %q, want %q", v.typ, store.CodeEntityFile)
		}
		if v.service != "auth-svc" {
			t.Errorf("service = %q, want %q", v.service, "auth-svc")
		}
	}

	if v, ok := callMap["app/payment/charge.go"]; !ok {
		t.Error("missing IncrementInvestigation call for app/payment/charge.go")
	} else {
		if v.typ != store.CodeEntityFile {
			t.Errorf("type = %q, want %q", v.typ, store.CodeEntityFile)
		}
		if v.service != "payment-svc" {
			t.Errorf("service = %q, want %q", v.service, "payment-svc")
		}
	}
}
