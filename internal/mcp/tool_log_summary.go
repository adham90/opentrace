...
func logSummaryHandler(ls store.LogStore, egs store.ErrorGroupStore) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		levelCounts, err := ls.CountByLevel(ctx, countParams)
		if err != nil {
		...
		// We use a generous limit to get a representative sample.
		...
		entries, err := ls.Search(ctx, searchParams)
		if err != nil {
		...
		for _, se := range sortedErrors[:limit] {
		...
		summaries, _ := ls.SearchRequestSummaries(ctx, summaryParams)
