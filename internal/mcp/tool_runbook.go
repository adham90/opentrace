...
func getQE(registry *connector.Registry) (connector.QueryExecutor, *mcp.CallToolResult) {
	ds := registry.Get(connector.ConnectorDatabase)
	if ds == nil {
		return nil, mcp.NewToolResultError("No database connector is active.")
	}
	qe, ok := ds.(connector.QueryExecutor)
	if !ok {
		return nil, mcp.NewToolResultError("active database connector does not support direct queries")
	}
	return qe, nil
}

func runbookSlowDB(ctx context.Context, registry *connector.Registry) (*mcp.CallToolResult, error) {
	qe, errResult := getQE(registry)
	if errResult != nil {
...
func runbookConnExhaustion(ctx context.Context, registry *connector.Registry) (*mcp.CallToolResult, error) {
	...
	if r, err := qe.ExecuteReadQuery(ctx, idleQ); err == nil && r.RowCount > 0 {
		idle := make([]map[string]any, 0)
		for _, row := range r.Rows {
			if len(row) >= 4 {
				idle = append(idle, map[string]any{
					"pid": row[0], "user": row[1], "app": row[2], "duration_sec": row[3],
				})
			}
		}
		sections["idle_in_transaction"] = idle
		diagnosis = append(diagnosis, fmt.Sprintf("%d idle-in-transaction sessions — likely leaked connections", len(idle)))
	}
