...
// requestPerformanceHandler returns a handler that queries request summaries
// for performance analysis — N+1 detection, slow requests, SQL-heavy endpoints.
func requestPerformanceHandler(ls store.LogStore) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		results, err := ls.SearchRequestSummaries(ctx, params)
		if err != nil {
