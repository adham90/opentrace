...
func TestMetricStore_BatchInsertAndQuery(t *testing.T) {
	...
	srv, err := ss.Register(ctx, store.RegisterServerParams{Hostname: "metric-test"})
	if err != nil {
	...
	points, err := ms.Query(ctx, store.MetricQuery{ServerID: srv.ID})
	if err != nil {
	...
	cpuPoints, err := ms.Query(ctx, store.MetricQuery{
		ServerID:   srv.ID,
...
func TestMetricStore_QueryTimeRange(t *testing.T) {
	...
	srv, _ := ss.Register(ctx, store.RegisterServerParams{Hostname: "time-range-test"})

	// Insert metrics at two different times
	t1 := time.Now().UTC().Add(-2 * time.Hour)
	t2 := time.Now().UTC()

	ms.BatchInsert(ctx, srv.ID, t1, []store.MetricSample{
		{Name: "cpu.usage_percent", Value: 30.0},
	})
	ms.BatchInsert(ctx, srv.ID, t2, []store.MetricSample{
		{Name: "cpu.usage_percent", Value: 80.0},
	})

	// Query only recent metrics
	oneHourAgo := time.Now().UTC().Add(-1 * time.Hour)
	points, err := ms.Query(ctx, store.MetricQuery{
		ServerID: srv.ID,
		Start:    &oneHourAgo,
	})
	if err != nil {
...
func TestMetricStore_LatestByServer(t *testing.T) {
	...
	srv, _ := ss.Register(ctx, store.RegisterServerParams{Hostname: "latest-test"})
...
func TestMetricStore_Prune(t *testing.T) {
	...
	srv, _ := ss.Register(ctx, store.RegisterServerParams{Hostname: "prune-test"})

	ts := time.Now().UTC()
	ms.BatchInsert(ctx, srv.ID, ts, []store.MetricSample{
	...
	points, _ := ms.Query(ctx, store.MetricQuery{ServerID: srv.ID})
	if len(points) != 0 {
...
func TestMetricStore_InvalidLabelsJSON(t *testing.T) {
	...
	srv, _ := ss.Register(ctx, store.RegisterServerParams{Hostname: "invalid-labels-test"})
...
func TestMetricStore_Labels(t *testing.T) {
	...
	srv, _ := ss.Register(ctx, store.RegisterServerParams{Hostname: "label-test"})
