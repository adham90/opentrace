...
func TestBatchInsert_Empty(t *testing.T) {
	...
	count, err := s.BatchInsert(context.Background(), []store.LogEntry{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if count != 0 {
...
func TestLogSearch_FTS(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{Query: "authentication"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 1 {
...
func TestLogSearch_FilterByService(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{Service: "worker"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 1 {
...
func TestLogSearch_FilterByLevel(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{Level: "ERROR"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 1 {
...
func TestLogSearch_TimeBounds(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{Start: &start, End: &end})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 2 {
...
func TestLogSearch_NoResults(t *testing.T) {
	...
	results, err := s.Search(context.Background(), store.LogSearchParams{Service: "nonexistent"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 0 {
...
func TestSearch_MetadataExact(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{MetadataFilter: map[string]string{"host": "server-01"}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 2 {
...
func TestSearch_MetadataContains(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{MetadataFilter: map[string]string{"path": "~users"}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 1 {
	...
	// Broader match
	results, err = s.Search(ctx, store.LogSearchParams{MetadataFilter: map[string]string{"path": "~/api"}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 2 {
...
func TestSearch_MetadataExists(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{MetadataFilter: map[string]string{"user_id": "*"}})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 2 {
...
func TestSearchRequestSummaries_NPlusOne(t *testing.T) {
	...
	results, err := s.SearchRequestSummaries(ctx, store.RequestSummarySearchParams{
		Start:         &start,
		End:           &end,
		NPlusOneOnly:  true,
	})
...
func TestBatchInsert_MetadataJSONSkipsMarshal(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{Limit: 1})
	if err != nil {
...
func TestLogStore_Prune(t *testing.T) {
	...
	results, err := s.Search(ctx, store.LogSearchParams{})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(results) != 1 {
