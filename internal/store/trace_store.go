...
func (s *traceStore) UpsertTraceStatus(ctx context.Context, traceID string, entry LogEntry) error {
	if traceID == "" {
		return nil
	}
...
func (s *traceStore) GetTraceStatus(ctx context.Context, traceID string) (*TraceStatus, error) {
	var ts TraceStatus
...
func (s *traceStore) ListRecentTraces(ctx context.Context, limit, offset int) ([]TraceStatus, int, error) {
	if limit <= 0 {
		limit = 50
	}
