...
func TestHealthCheckStore_CreateAndGet(t *testing.T) {
	...
	hc, err := s.Create(ctx, CreateHealthCheckParams{
		Name: "My API",
...
func TestHealthCheckStore_Delete(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "ToDelete", URL: "https://del.com"})
	...
	if err := s.Delete(ctx, "nonexistent"); err != ErrNotFound {
...
func TestHealthCheckStore_SetEnabled(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "Toggle", URL: "https://toggle.com"})
	...
	if err := s.SetEnabled(ctx, "nonexistent", true); err != ErrNotFound {
...
func TestHealthCheckStore_RecordAndLatestResults(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "Results", URL: "https://results.com"})
	...
	now := time.Now().UTC()
	s.RecordResult(ctx, HealthCheckResult{
		HealthCheckID: hc.ID,
		Status:        HealthCheckUp,
		StatusCode:    &code200,
		ResponseMs:    &ms50,
	...
	// Most recent first
	if results[0].Status != HealthCheckDegraded {
		t.Errorf("results[0].Status = %q, want degraded", results[0].Status)
	}
	if results[1].Status != HealthCheckDown {
...
func TestHealthCheckStore_UptimeSummaries(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "Uptime", URL: "https://uptime.com"})
...
func TestHealthCheckStore_PruneResults(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "Prune", URL: "https://prune.com"})
...
func TestHealthCheckStore_DeleteCascadesResults(t *testing.T) {
	...
	hc, _ := s.Create(ctx, CreateHealthCheckParams{Name: "Cascade", URL: "https://cascade.com"})
	code200 := 200
	ms50 := 50
	s.RecordResult(ctx, HealthCheckResult{
		HealthCheckID: hc.ID,
