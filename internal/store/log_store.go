...
func (s *logStore) BatchInsert(ctx context.Context, entries []LogEntry) (int, error) {
	...
	stmt, err := tx.PrepareContext(ctx,
		`INSERT INTO logs (timestamp, level, service, trace_id, message, environment, event_type, metadata)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
...
func (s *logStore) Search(ctx context.Context, params LogSearchParams) ([]LogEntry, error) {
	...
	var query string
	if useFTS {
		query = `SELECT l.id, l.timestamp, l.level, l.service, l.trace_id, l.message, l.environment, l.event_type, l.metadata
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&entry.TraceID, &entry.Message, &entry.Environment, &entry.EventType, &metaJSON,
		); err != nil {
...
func (s *logStore) GetByID(ctx context.Context, id int64) (*LogEntry, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, timestamp, level, service, trace_id, message, environment, event_type, metadata FROM logs WHERE id = ?`, id)

	var entry LogEntry
	var tsStr string
	var metaJSON sql.NullString
	if err := row.Scan(&entry.ID, &tsStr, &entry.Level, &entry.Service,
		&entry.TraceID, &entry.Message, &entry.Environment, &entry.EventType, &metaJSON); err != nil {
