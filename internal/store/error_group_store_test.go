...
func TestErrorGroupStore_Upsert_NewGroup(t *testing.T) {
	...
	if eg.Status != ErrorGroupUnresolved {
...
func TestErrorGroupStore_Upsert_SkipsEmptyFingerprint(t *testing.T) {
	...
	entry := LogEntry{ID: 0, Level: "ERROR", Service: "myapp", Timestamp: time.Now()}
	if err := s.Upsert(ctx, entry); err != nil {
		t.Fatalf("Upsert: %v", err)
	}
...
func TestErrorGroupStore_Resolve_And_Reopen(t *testing.T) {
	...
	if eg.Status != ErrorGroupResolved {
...
func TestErrorGroupStore_Ignore_StaysIgnored(t *testing.T) {
	...
	if eg.Status != ErrorGroupIgnored {
...
func TestErrorGroupStore_List(t *testing.T) {
	...
	groups, err := s.List(ctx, ListErrorGroupParams{})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(groups) != 3 {
	...
	unresolved, err := s.List(ctx, ListErrorGroupParams{Status: ErrorGroupUnresolved})
	if err != nil {
...
func TestErrorGroupStore_Count(t *testing.T) {
	...
	unresolved, err := s.Count(ctx, ErrorGroupUnresolved)
	if err != nil {
