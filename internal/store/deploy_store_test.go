...
func setupDeployStore(t *testing.T) DeployStore {
	t.Helper()
	db := setupTestDB(t)
...
func TestDeployCreate(t *testing.T) {
	...
	d, err := s.Create(ctx, CreateDeployParams{
		Service:      "api",
		...
		DeploySource: DeploySourceWebhook,
	})
	if err != nil {
...
func TestDeployGetByCommit(t *testing.T) {
	...
	_, err := s.Create(ctx, CreateDeployParams{
		Service:    "api",
		CommitHash: "xyz789",
	})
	if err != nil {
...
func TestDeployMeasureImpact(t *testing.T) {
	...
	err = s.MeasureImpact(ctx, d.ID, DeployImpact{
		PreErrorRate:     0.01,
		PostErrorRate:    0.05,
	...
	if updated.Status != DeployStatusIncident {
