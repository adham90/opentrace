...
func NewTestCorrelationStore(db *sql.DB) TestCorrelationStore {
...
func (s *testCorrelationStore) TopByPriority(ctx context.Context, service string, limit int) ([]UncoveredErrorPath, error) {
	if limit <= 0 {
		limit = 10
	}
...
func (s *testCorrelationStore) GetByFingerprint(ctx context.Context, fingerprint string) (*UncoveredErrorPath, error) {
...
func scanUncoveredPath(rows *sql.Rows) (*UncoveredErrorPath, error) {
	var p UncoveredErrorPath
	var lastSeenAt, createdAt, updatedAt string
	err := rows.Scan(
		&p.ID, &p.Service, &p.ErrorFingerprint, &p.ErrorClass,
