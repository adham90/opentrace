...
func NewJourneyStore(db *sql.DB) JourneyStore {
...
func (s *journeyStore) GetSession(ctx context.Context, sessionID string) (*UserSession, error) {
	var us UserSession
...
func (s *journeyStore) ListSessions(ctx context.Context, params SessionListParams) ([]UserSession, error) {
...
func (s *journeyStore) GetSessionRequests(ctx context.Context, sessionID string) ([]RequestStep, error) {
	rows, err := s.db.QueryContext(ctx, `
...
func (s *journeyStore) GetUserJourney(ctx context.Context, userID string, since time.Time, limit int) ([]RequestStep, error) {
	if limit <= 0 {
		limit = 100
	}
	rows, err := s.db.QueryContext(ctx, `
...
func scanRequestSteps(rows *sql.Rows) ([]RequestStep, error) {
	var results []RequestStep
	for rows.Next() {
...
func (s *journeyStore) CommonPaths(ctx context.Context, params PathAnalysisParams) ([]PathFrequency, error) {
	...
	var results []PathFrequency
	for _, p := range pathCounts {
		if p.count < params.MinOccurrences {
			continue
		}
		results = append(results, PathFrequency{
			Steps:            p.steps,
			Count:            p.count,
...
func (s *journeyStore) CreateFunnel(ctx context.Context, funnel Funnel) (*Funnel, error) {
...
func (s *journeyStore) GetFunnel(ctx context.Context, id int64) (*Funnel, error) {
	var f Funnel
	var stepsJSON, createdStr, updatedStr string
	err := s.db.QueryRowContext(ctx, `
...
func (s *journeyStore) ListFunnels(ctx context.Context) ([]Funnel, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, name, service, steps, created_at, updated_at
	...
	for rows.Next() {
		var f Funnel
		var stepsJSON, createdStr, updatedStr string
		if err := rows.Scan(&f.ID, &f.Name, &f.Service, &stepsJSON, &createdStr, &updatedStr); err != nil {
			return nil, fmt.Errorf("scanning funnel: %w", err)
		}
		json.Unmarshal([]byte(stepsJSON), &f.Steps)
		f.CreatedAt, _ = time.Parse(time.RFC3339, createdStr)
		f.UpdatedAt, _ = time.Parse(time.RFC3339, updatedStr)
...
func (s *journeyStore) AnalyzeFunnel(ctx context.Context, funnelID int64, since time.Time) (*FunnelResult, error) {
	funnel, err := s.GetFunnel(ctx, funnelID)
	if err != nil {
		return nil, err
	}
	if len(funnel.Steps) == 0 {
		return &FunnelResult{FunnelName: funnel.Name}, nil
	}
...
func (s *journeyStore) DeleteFunnel(ctx context.Context, id int64) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM funnels WHERE id = ?`, id)
	if err != nil {
...
func (s *journeyStore) GetRequestTimeline(ctx context.Context, logID int64) (*RequestTimeline, error) {
	var rt RequestTimeline
	var tsStr string
	var timeline, path sql.NullString
	err := s.db.QueryRowContext(ctx, `
		SELECT l.id, COALESCE(l.request_id, ''), COALESCE(rs.controller, ''),
		       COALESCE(rs.action, ''), COALESCE(rs.path, ''),
		       COALESCE(rs.status, 0), COALESCE(rs.duration_ms, 0),
	...
	rt.StartedAt, _ = time.Parse(time.RFC3339Nano, tsStr)
	if path.Valid {
		rt.Path = path.String
	}

	// Parse timeline JSON
	if timeline.Valid && timeline.String != "" {
		rt.Events = parseTimelineJSON(timeline.String)
	}

	// Find bottleneck
	if len(rt.Events) > 0 {
		var slowest *TimelineEvent
...
func (s *journeyStore) GetSessionTimeline(ctx context.Context, sessionID string) ([]RequestTimeline, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT l.id, COALESCE(l.request_id, ''), COALESCE(rs.controller, ''),
		       COALESCE(rs.action, ''), COALESCE(rs.path, ''),
		       COALESCE(rs.status, 0), COALESCE(rs.duration_ms, 0),
	...
	var results []RequestTimeline
	for rows.Next() {
		var rt RequestTimeline
		var tsStr string
		var timeline, path sql.NullString
		if err := rows.Scan(&rt.LogID, &rt.RequestID, &rt.Controller, &rt.Action, &path,
			&rt.Status, &rt.DurationMs, &tsStr, &timeline); err != nil {
			return nil, fmt.Errorf("scanning session timeline: %w", err)
		}
		rt.StartedAt, _ = time.Parse(time.RFC3339Nano, tsStr)
		if path.Valid {
			rt.Path = path.String
...
func parseTimelineJSON(raw string) []TimelineEvent {
