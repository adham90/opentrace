...
func TestWatchStore_CRUD(t *testing.T) {
	...
	// Create
	w, err := ws.Create(ctx, CreateWatchParams{
		Metric:    WatchMetricErrorRate,
		Operator:  WatchOpGreaterThan,
		Threshold: 0.05,
		Service:   "web",
	})
	if err != nil {
	...
	if w.Metric != WatchMetricErrorRate {
		t.Errorf("metric = %q, want %q", w.Metric, WatchMetricErrorRate)
	}
	...
	// List
	list, err := ws.List(ctx, ListWatchParams{})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(list) != 1 {
	...
	// List with filter
	filtered, err := ws.List(ctx, ListWatchParams{Service: "other"})
	if err != nil {
	...
	// UpdateStatus
	err = ws.UpdateStatus(ctx, w.ID, WatchStatusTriggered)
	if err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}
	got, _ = ws.GetByID(ctx, w.ID)
	if got.Status != WatchStatusTriggered {
...
func TestWatchStore_Defaults(t *testing.T) {
	...
	if w.Urgency != WatchUrgencyNormal {
...
func TestWatchStore_ExpireWatches(t *testing.T) {
	...
	list, _ := ws.List(ctx, ListWatchParams{Status: WatchStatusExpired})
	if len(list) != 1 {
...
func TestWatchStore_BaselineRoundTrip(t *testing.T) {
	...
	w, err := ws.Create(ctx, CreateWatchParams{
		Metric:    WatchMetricResponseTime,
		Operator:  WatchOpGreaterThan,
		Threshold: 200,
		Service:   "api",
	})
	...
	baseline := &WatchBaseline{
		...
		Endpoints: []WatchEndpointBaseline{
...
func TestWatchStore_RunCRUD(t *testing.T) {
	...
	w, _ := ws.Create(ctx, CreateWatchParams{
		Metric:    WatchMetricErrorRate,
		Operator:  WatchOpGreaterThan,
		Threshold: 0.1,
	})
...
func TestWatchStore_AlertCRUD(t *testing.T) {
	...
	// Create alert
	alert, err := ws.CreateAlert(ctx, CreateWatchAlertParams{
		WatchID:        w.ID,
		Urgency:        WatchUrgencyHigh,
		Summary:        "error rate spike",
		TriggerMetric:  "error_rate",
		TriggerValue:   0.12,
		ThresholdValue: 0.05,
		Evidence: &WatchEvidenceBundle{
...
func TestWatchStore_NotFound(t *testing.T) {
	...
	err = ws.UpdateStatus(ctx, "nonexistent", WatchStatusTriggered)
	if err != ErrNotFound {
