...
func TestEventStore_CreateAndGetByID(t *testing.T) {
	...
	e, err := s.Create(ctx, CreateEventParams{
	...
	if e.EventType != EventTypePR {
		t.Errorf("EventType = %q, want %q", e.EventType, EventTypePR)
	}
...
func TestEventStore_List(t *testing.T) {
	...
	s.Create(ctx, CreateEventParams{EventType: EventTypePR, Service: "api", Title: "PR 1"})
	s.Create(ctx, CreateEventParams{EventType: EventTypeTest, Service: "api", Title: "Test 1"})
	s.Create(ctx, CreateEventParams{EventType: EventTypePR, Service: "web", Title: "PR 2"})

	// List all
	all, err := s.List(ctx, ListEventParams{})
	if err != nil {
	...
	// Filter by type
	prs, err := s.List(ctx, ListEventParams{EventType: EventTypePR})
	if err != nil {
	...
	// Filter by service
	apiEvents, err := s.List(ctx, ListEventParams{Service: "api"})
	if err != nil {
...
func TestEventStore_GetByExternalID(t *testing.T) {
	...
	s.Create(ctx, CreateEventParams{EventType: EventTypePR, ExternalID: "pr-42", Title: "PR 42"})

	got, err := s.GetByExternalID(ctx, EventTypePR, "pr-42")
	if err != nil {
	...
	_, err = s.GetByExternalID(ctx, EventTypePR, "nonexistent")
	if err != ErrNotFound {
...
func TestEventStore_Prune(t *testing.T) {
	...
	s.Create(ctx, CreateEventParams{EventType: EventTypeCommit, Title: "Commit 1"})
...
func TestEventStore_ListWithSince(t *testing.T) {
	...
	s.Create(ctx, CreateEventParams{EventType: EventTypeAlert, Service: "api", Title: "Alert 1"})

	// Since = future, should find nothing
	future := time.Now().Add(1 * time.Hour)
	events, err := s.List(ctx, ListEventParams{Since: future})
	if err != nil {
