...
func (s *eventStore) Create(ctx context.Context, params CreateEventParams) (*Event, error) {
	metadataJSON := "{}"
...
func (s *eventStore) List(ctx context.Context, params ListEventParams) ([]Event, error) {
	var where []string
	var args []any
...
func (s *eventStore) GetByExternalID(ctx context.Context, eventType EventType, externalID string) (*Event, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, event_type, source, service, title, description, metadata_json, external_id, external_url, author, created_at
