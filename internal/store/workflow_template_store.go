...
func NewWorkflowTemplateStore(db *sql.DB) WorkflowTemplateStore {
	return &workflowTemplateStore{db: db}
}

func (s *workflowTemplateStore) Seed(ctx context.Context, templates []WorkflowTemplate) error {
...
func (s *workflowTemplateStore) GetNextStep(ctx context.Context, intent string, stepOrder int) ([]WorkflowTemplate, error) {
...
func (s *workflowTemplateStore) GetByName(ctx context.Context, name string) ([]WorkflowTemplate, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, intent, name, step_order, tool_name, args_hint, source, resolved_session_count, created_at
		 FROM workflow_templates
		 WHERE name = ?
...
func (s *workflowTemplateStore) List(ctx context.Context, intent string) ([]WorkflowTemplate, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, intent, name, step_order, tool_name, args_hint, source, resolved_session_count, created_at
		 FROM workflow_templates
		 WHERE intent = ?
