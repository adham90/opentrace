...
func TestErrorImpactStore_GetUserErrors(t *testing.T) {
	...
	ls.BatchInsert(ctx, []LogEntry{entry})
...
func TestErrorImpactStore_TopByImpact(t *testing.T) {
	...
	ls.BatchInsert(ctx, []LogEntry{entryA})
	...
	ls.BatchInsert(ctx, []LogEntry{entryB})
	...
	// Query top by impact
	results, err := eis.TopByImpact(ctx, ImpactQueryParams{
