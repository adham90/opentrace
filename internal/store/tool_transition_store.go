...
func NewToolTransitionStore(db *sql.DB) ToolTransitionStore {
...
func (s *toolTransitionStore) IncrementWithOutcome(ctx context.Context, fromTool, toTool, intent, outcome string) error {
	...
	switch outcome {
	case string(InvestigationStatusResolved):
		resolvedInc = 1
	case string(InvestigationStatusAbandoned):
		abandonedInc = 1
	}
...
func (s *toolTransitionStore) GetTransitions(ctx context.Context, params GetTransitionsParams) ([]ToolTransition, error) {
	...
	var result []ToolTransition
	for rows.Next() {
		var t ToolTransition
...
func (s *toolTransitionStore) GetDeadEnds(ctx context.Context, intent string) ([]ToolTransition, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT from_tool, to_tool, intent, total_count, resolved_count, abandoned_count, avg_duration_ms, last_seen_at
		 FROM tool_transitions
