...
func NewTrendStore(db *sql.DB) TrendStore {
...
func (s *trendStore) AggregateBuckets(ctx context.Context, interval string, since time.Time) error {
	...
	for t := since; t.Before(now); t = t.Add(bucketDur) {
		...
		merged := make(map[bucketKey]*MetricBucket)
...
func (s *trendStore) QueryTrends(ctx context.Context, params TrendQueryParams) ([]MetricBucket, error) {
...
func (s *trendStore) ListDeployMarkers(ctx context.Context, service string, since time.Time) ([]DeployMarker, error) {
	var conditions []string
	var args []any
