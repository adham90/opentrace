...
func TestUserStore_GetByEmail(t *testing.T) {
	...
	_, err := us.Create(ctx, CreateUserParams{
...
func TestUserStore_GetByMCPToken(t *testing.T) {
	...
	trueVal := true
	_, err = us.Update(ctx, user.ID, UpdateUserParams{MCPEnabled: &trueVal})
	if err != nil {
	...
	falseVal := false
	_, err = us.Update(ctx, user.ID, UpdateUserParams{MCPEnabled: &falseVal})
	if err != nil {
	...
	_, err = us.Update(ctx, user.ID, UpdateUserParams{IsActive: &falseVal})
	if err != nil {
...
func TestUserStore_Update(t *testing.T) {
	...
	updated, err := us.Update(ctx, user.ID, UpdateUserParams{
		DisplayName: &newName,
		Role:        &newRole,
		MCPEnabled:  &trueVal,
	})
	...
	if updated.Role != RoleMember {
		t.Errorf("Role = %q, want %q", updated.Role, RoleMember)
	}
	...
	// Deactivate user.
	falseVal := false
	updated, err = us.Update(ctx, user.ID, UpdateUserParams{IsActive: &falseVal})
	if err != nil {
...
func TestUserStore_Count(t *testing.T) {
	...
	_, _ = us.Create(ctx, CreateUserParams{
...
func TestUserStore_LastAdminProtection(t *testing.T) {
	...
	admin1, err := us.Create(ctx, CreateUserParams{
	...
	_, err = us.Update(ctx, admin1.ID, UpdateUserParams{Role: &memberRole})
	if err != ErrLastAdmin {
